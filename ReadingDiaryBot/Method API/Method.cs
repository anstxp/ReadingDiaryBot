...
namespace ReadingDiaryBot.Method_API
{
    ...
    new[]
    {
        ...
        public async Task GetBookDB(ITelegramBotClient botClient, Message message, string UserID, string Title)
        {
            ...
            if (result != null)
            {
                string allAuthors = string.Join(", ", result.Author);
                string allTags = string.Join(", ", result.PersonalTag);
                string allCategories = string.Join(", ", result.Genre);
        ...
        public async Task GetAll(ITelegramBotClient botClient, Message message)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                // Виведення кожної книги в окремому повідомленні
